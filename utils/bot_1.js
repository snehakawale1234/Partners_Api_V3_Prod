...
module.exports = {
    ...
    isMobileInternational: function (number) {
        ...
        try {
            //   console.log({ number });
            let mobileNumber = phoneUtil.parse(number, '');
            if (phoneUtil.isValidNumber(mobileNumber) && (phoneUtil.getNumberType(mobileNumber) == 1 || phoneUtil.getNumberType(mobileNumber) == 2)) {
                isValid = true;
            }
        } catch (e) {
    ...
    callWhatsAppApi1: function (instanceUrl, data, httpMethod, requestType, apiHeaders) {
    ...
    isMobileInternationalWebhookUrl: function (url, attributes) {
        var pattern = /[^{{\}]+(?=}})/g;
    ...
    uploadWhatsappMediaCloud: function (instanceUrl, data, apiHeaders) {
        console.log("============inside uploadWhatsappMediaCloud================");
        ...
        return new Promise(function (resolve, reject) {
            ...
            try {
                ...
                // console.log("response reject error", e);
                reject(e);
            }
        });
    },
    ...
    deleteWhatsappTemplate: function (data, whatsappAccountId, apiHeaders) {
        ...
        let url = `${instanceUrl}/v11.0/${whatsappAccountId}/message_templates?${qs.stringify(data)}`;
        let headers = {
            'cache-control': 'no-cache'
        };
        let len = apiHeaders.length;
    ...
    fetchPlaceholders: function (placeholders) {
        let placeholderArr = JSON.parse(placeholders);
        let tempArr = [];
