...
module.exports = async (req, res) => {
    try {
        ...
        var authentication_payload = null;
        let userid = null;
        ...
        let securityrecommendation = null;
        let expirytimeforthecode;
        let expiresin;
        ...
        if (getusersv3[0][0] === undefined) {
            ...
            if (category.toLowerCase() === "authentication") {
                if (req.body.message_send_ttl_seconds != undefined) {
                    objData["message_send_ttl_seconds"] = req.body.message_send_ttl_seconds;
                }
            }
            ...
            if (create_templateresult.response != undefined) {
                ...
                if (category.toLowerCase() === "authentication") {
                    ...
                    if (req.body.message_send_ttl_seconds != undefined) {
                        authentication_payload.msgvalidityperiod = 0;
                        authentication_payload.validityperiod = req.body.message_send_ttl_seconds;
                    } else {
                        authentication_payload.msgvalidityperiod = 1;
                        authentication_payload.validityperiod = "30days";
                    }
                    authentication_payload.expirytimeforthecode = 1;
                    authentication_payload.expiresin = 0;
                }

                if (ComponentsData.findIndex(x => x.type.toLowerCase() === "body") > -1) {
                    if (category.toLowerCase() === "authentication") {
                        let bodyindex = ComponentsData.findIndex(x => x.type.toLowerCase() === "body");
                        let is_securityrecommendation = ComponentsData[bodyindex].add_security_recommendation;
                        if (is_securityrecommendation) {
                            authentication_payload.securityrecommendation = 0;
                        } else {
                            authentication_payload.securityrecommendation = 1;
                        }
                    } else {
                        let bodyindex = ComponentsData.findIndex(x => x.type.toLowerCase() === "body");
                ...
                if (ComponentsData.findIndex(x => x.type.toLowerCase() === "footer") > -1) {
                    if (category.toLowerCase() === "authentication") {
                        ...
                        footer_text = ComponentsData[footerindex].code_expiration_minutes != undefined ? ComponentsData[footerindex].code_expiration_minutes : 0;
                        if (footer_text != null) {
                            authentication_payload.expiresin = footer_text;
                            authentication_payload.expirytimeforthecode = 0;
                        }
                    } else {
                        let footerindex = ComponentsData.findIndex(x => x.type.toLowerCase() === "footer");
                ...
                if (ComponentsData.findIndex(x => x.type.toLowerCase() === "buttons") > -1) {
                    ...
                    for (let i = 0; i < urldata.length; i++) {
                        ...
                        if (urldata[i].type.toLowerCase() === "otp") {
                            if (urldata[i].otp_type.toLowerCase() === "copy_code") {
                                authentication_payload.autofillcopycode = "1";
                                authentication_payload.copycodeinput = urldata[i].text;
                            }
                            if (urldata[i].otp_type.toLowerCase() === "one_tap") {
                                authentication_payload.autofillcopycode = "0";
                                authentication_payload.zerotaponoff = 0;
                                authentication_payload.zerotapcheckbox = 0;
                                authentication_payload.copycodeinput = urldata[i].text;
                                authentication_payload.autofillinpupt = urldata[i].autofill_text;
                                authentication_payload.zero_tap_terms_accepted = 1;
                                authentication_payload.packagename = urldata[i].supported_apps[0].package_name;
                                authentication_payload.appsighash = urldata[i].supported_apps[0].signature_hash;

                            }
                            if (urldata[i].otp_type.toLowerCase() === "zero_tap") {
                                authentication_payload.autofillcopycode = "0";
                                authentication_payload.zerotaponoff = 1;
                                authentication_payload.zerotapcheckbox = 1;
                                authentication_payload.copycodeinput = urldata[i].text;
                                authentication_payload.autofillinpupt = urldata[i].autofill_text;
                                authentication_payload.zero_tap_terms_accepted = 1;
                                authentication_payload.packagename = urldata[i].supported_apps[0].package_name;
                ...
                // console.log("==============>authentication", authentication_payload);
                if (category.toLowerCase() === "authentication") {
                    let insertTemplateResult = await userService1.insertTemplateDetailsV2(responceaxios.data, datares, flowid1, carousel_payload, category_change, type_of_marketing, marketing_template_format, head_media_url, status112, lto_expiration_time_flag, ltotext, authentication_payload);
                } else {
